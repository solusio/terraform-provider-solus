...
func TestAccResourcePlan(t *testing.T) {
	...
	checker := func(
		...
		return resource.ComposeTestCheckFunc(
			...
			resource.TestCheckResourceAttr(resName, "params.0.ram_mb", strconv.Itoa(params.RAM/Mb)),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Create resource with all default values.
			{
				Config: fmt.Sprintf(
					`
resource "solus_plan" "%[1]s_default" {
	name = "%[1]s_default"
...
`,
				...
				Check: checker(
					fmt.Sprintf("solus_plan.%s_default", name),
					name+"_default",
...
`,
				...
				Check: resource.ComposeTestCheckFunc(
					checker(
						fmt.Sprintf("solus_plan.%s_some", name),
						name+"_some",
						solus.StorageTypeNameFB,
						solus.ImageFormatRaw,
						solus.PlanParams{
...
`,
				...
				Check: resource.ComposeTestCheckFunc(
					checker(
						fmt.Sprintf("solus_plan.%s_full", name),
						name+"_full",
						solus.StorageTypeNameFB,
						solus.ImageFormatRaw,
						solus.PlanParams{
...
`,
			...
			// Update plan's field which should force creating new one.
			{
				Config: fmt.Sprintf(
					`
resource "solus_plan" "%[1]s_full" {
